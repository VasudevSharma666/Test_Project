...
const HomePage = () => {
        ...
        {state.posts.map((post) => (
          <div key={post.id}>
            <div className="PostsTag">
              ...
              <Link id={post.id} to={commt} className="LinkToComment">
                See The Comment
              </Link>
              <input
                type="image"
                id={post.id}
                onClick={(e) => DeletedThePost(e)}
  ...
  const NullPosts = () => {
    setTimeout(() => {
      dispatch({ type: "PostsLen", value: true });
    }, 3000);
    if (state.PostsLen) {
    ...
    <React.Fragment>
      <div className="HomePageBackground">
        ...
        <div className="StartPoint">
          {state.posts.length > 0 ? <ShowTheData /> : <NullPosts />}
        </div>
      </div>
    </React.Fragment>
  );
